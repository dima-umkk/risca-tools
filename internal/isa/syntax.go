...
// Example: -Number; make negative number; +Number; + ignored
func parsePlusMinusNumber(parser *Parser, rule Rule, tokens []Token, tokenpos int) ([]Token, error) {
	plmnT := tokens[tokenpos]
	numT := tokens[tokenpos+1]
	value := numT.ValInt
	if plmnT.Tk == "-" { //make negative number
		value = -value
	} //+ ignored
	//Remove second token and replace first by number
	tokens[tokenpos] = Token{T: TK_NUMBER, Tk: strconv.Itoa(int(value)), ValInt: int32(value)}
	tokens = append(tokens[:tokenpos+1], tokens[tokenpos+2:]...)
	return tokens, nil
}
